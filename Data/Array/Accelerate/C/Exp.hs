...
elet env aenv bnd body
  = map (\bodyiC -> [cexp| ({ $items:inits $exp:bodyiC; }) |]) (openExpToC env_t aenv body)
  where
    (binders, env_t) = env `pushExpEnv` bnd
    inits            = zipWith bindOneComponent binders (openExpToC env aenv bnd)
...
tupToC env  aenv  (SnocTup t e) = tupToC env aenv t ++ openExpToC env aenv e
...
condToC env aenv p t e
  = zipWith (\tiC eiC -> [cexp| $exp:pC ? $exp:tiC : $exp:eiC |]) (openExpToC env aenv t) (openExpToC env aenv e)
  where
